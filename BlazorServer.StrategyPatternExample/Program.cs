using BlazorServer.StrategyPatternExample.Data;
...
builder.Services.AddDbContext<StrategyPatternContext>(options => options.UseSqlite(builder.Configuration.GetConnectionString("StrategyPattern")));
...
async Task EnsureDbIsMigrated(IServiceProvider services)
{
    ...
    using var ctx = scope.ServiceProvider.GetService<StrategyPatternContext>();
    if (ctx != null)
    {
